...
def main():
   ...
   # LoRA configuration
    ...
    config = PPOConfig(
        ...
        project_kwargs={"logging_dir": './output_RLHF_norm'}
    )
    ...
    # Generation parameters for reward model
    BATCH = 32
    ...
    # generation parameters for ppo model
    ...
    # Normalize model output so that initial rewards have mean of 0 and std of 1

    r_gain = 1
    r_bias = 0

    # get on batch of queries and generate responses to normalize rewards
    for batch in ppo_trainer.dataloader:
        ...
        init_rewards = [torch.tensor(output[0]["score"] * r_gain + r_bias) for output in pipe_outputs]

        # calculate mean and std to normalize model rewards
        r_mean = np.mean(init_rewards)
        r_std = np.std(init_rewards)
    ...
    for i in range(BATCH):
        prompt_list.append(dataset['eval'][i]['text'])
        chosen_list.append(dataset['eval'][i]['baseline_summary'])
        rejected_list.append(dataset['eval'][i]['model_summary'])
    ...
    # Start training loop to run reinforcement learning
    ...
    plt.savefig('ppo_reward_norm_e20_v2.png')
    ...
    ppo_model.push_to_hub("GPT1B_domar_RLHF_e20_v2")
